...
export const globalSlice = createSlice({
  ...
  reducers: {
    ...
    setChatShow: (state, action: PayloadAction<boolean>) => {
    ...
    addChatMessage: (state, action: PayloadAction<IChatMessage>) => {
      ...
      const exists = state.chatMessages.some((msg) => {
        // Check by ID first (most reliable)
        if (msg.id === action.payload.id) {
          return true
        }
        // Also check by content, userId, and similar timestamp (within 3 seconds)
        return (
          msg.userId === action.payload.userId &&
          msg.content === action.payload.content &&
          Math.abs(msg.timestamp - action.payload.timestamp) < 3000
        )
      })
      if (!exists) {
