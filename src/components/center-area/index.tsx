...
const CenterArea = (props: ICenterAreaProps) => {
  ...
  const videoMute = useMemo(() => {
    if (data.isLocal) {
      // If screen sharing, video is not muted
      if (data.screenTrack) {
        return false
      }
      return localVideoMute
    }
    return !data.videoTrack && !data.screenTrack
  }, [localVideoMute, data])
      ...
      {videoMute ? (
        ...
        <div className={styles.videoMute}>
          ...
          <div className={styles.streamPlayerWrapper}>
            {data.isLocal ? (
              <LocalStreamPlayer videoTrack={data.videoTrack} screenTrack={data.screenTrack}></LocalStreamPlayer>
            ) : (
              <RemoteStreamPlayer
